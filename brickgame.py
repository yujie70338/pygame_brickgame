...
class Ball(pygame.sprite.Sprite):  # 球體角色
    ...
    direction = 0  # 球移動方向
    speed = 0  # 球移動速度
    # __init__ 為類別建構式，類別中一定要有此函式，建立物件時會執行此函式，僅會執行一次，用於建立圖形、載入圖片、初始化變數等等
    ...
    def update(self):  #球體移動
        ...
        self.dx = self.speed * math.cos(radian)  # 水平運動速度
        self.dy = -self.speed * math.sin(radian)  # 垂直運動速度
        ...
        if(self.rect.left <= 0 or self.rect.right >= screen.get_width()-10):  # 到達左右邊界
            ...
            self.rect.top = 10 # 在碰到上邊界時設定其y座標為10，避免連續碰撞上邊界
            self.bounceup()
    ...
    def bouncelr(self):  # 左右邊界反彈 # 除以360，取餘數避免負值
...
class Pad(pygame.sprite.Sprite):  # 滑板角色
    def __init__(self):
        ...
        self.image.convert() # convert 增加繪製的速度
        self.rect = self.image.get_rect()
...
for row in range(0, 4):  # 3列方塊
    for column in range(0, 15):  # 每列15磚塊
        if row == 0 or row == 1:  # 1,2列為綠色磚塊
            brick = Brick((0,255,0), column * 40 + 1, row * 15 + 1) # 保留兩像素做為磚塊區隔
...
while running:
    ...
    if playing == True:  # 遊戲進行中
        screen.blit(background, (0,0))  # 清除繪圖視窗 # 從左上角(0,0)開始繪製
